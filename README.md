...
- 🎬 **单镜头最长 30 秒**：单个分镜时长可达 30 秒（6–30 秒可调），长镜头叙事更连贯，最终成片总时长上限 `1200` 秒。
...
- 🎭 **角色装扮 / 场景状态变体**：按分镜自动派生角色装扮图与场景时间/天气状态图，并在故事版、分镜视频中一致引用。
...
- 🤖 **多 Agent 全自动流水线**：剧本 → 参考图库 → 角色装扮/场景状态变体 → 分镜故事版 → 分镜视频生成与审核 → 长视频合成，一站式闭环。
...
```text
需求输入 -> 剧本生成 -> 参考图库确认 -> 角色装扮/场景状态变体 -> 分镜故事版生成与审核 -> 连环画 PDF 导出 + 分镜视频生成与审核 -> 最终合成
```
...
- 布景设定上限：`30`
- 相邻分镜要求连贯承接，但不得重复复述
- 分镜中角色的特殊装扮写入“角色动作部分”，场景的时间/天气状态写入“场景描述部分”，并持久化用于后续生图/生视频
- 大模型原始响应会输出到服务端日志
...
- 仅当分镜的“角色动作部分”包含与默认装扮不同的角色装扮，或“场景描述部分”包含与布景默认不同的时间/天气状态时才生成
- 角色装扮图 = 该分镜的角色装扮描述 + 对应角色主图
- 场景状态图 = 该分镜的时间/天气状态 + 对应场景主图
